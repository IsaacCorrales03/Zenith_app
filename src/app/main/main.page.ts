...
import { CrearGrupoForm } from '../components/create-group-form/create-group-form'
...
export class MainPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    // Inicializar el servicio de API con los datos del usuario
    this.apiService.initialize(userData);
  ...
  async obtenerDatosDelUsuario() {
    ...
    this.apiService.obtenerDatosDeUsuario().subscribe({
      next: async (datos) => {
        ...
        this.loading = false;
        console.log(this.datos_del_usuario)
      },
      error: (err) => {
  ...
  async enrolledUser(curso_id: number) {
    this.error = null;
    console.log("enrolling user");

    const data = {};

    this.apiService.enrollUser(data, curso_id).subscribe({
      next: async (response) => {
        ...
        await toast.present();
        this.tabs.select('progress');
        // Navegar a progress después que se muestre el mensaje
        setTimeout(() => {
      ...
      error: async (error) => {
        ...
        const toast = await this.toastCtrl.create({
          message: 'Error al inscribirte: ' + (error.message || 'Intenta nuevamente'),
          duration: 3000,
          position: 'bottom',
          color: 'danger'
        });
